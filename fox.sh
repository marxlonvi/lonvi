...
CONFIG="$HOME/.fox"
QUEUEFILE="$HOME/.fox_queue"
PIDFILE="$HOME/.fox.pid"
...
RAMPIDFILE="$HOME/.fox_ramboost.pid"
...
# (misal lewat `su -c "bash fox.sh"`). Dalam kondisi ini kita TIDAK
...
(curl -fsSL "$GITHUB/version.txt" 2>/dev/null | {
    ...
    if [ -n "$remote" ] && [ "$remote" != "$VERSION" ]; then
        tmpfile=$(mktemp)
        curl -fsSL "$GITHUB/fox.sh" -o "$tmpfile" 2>/dev/null && \
...
update_status_cache() {
    ...
    if [ -f "$HOME/.fox_cache.pid" ] 2>/dev/null && kill -0 "$(cat "$HOME/.fox_cache.pid" 2>/dev/null)" 2>/dev/null; then
        CACHE_ACTIVE=1
    fi
        ...
        if [ "$total" -gt 0 ]; then
            ...
            if [ "$running" -lt "$total" ]; then
                if command -v termux-notification >/dev/null 2>&1; then
                    termux-notification --id fox_launcher --title "FOX Auto-Launcher" \
...
stop_launcher() {
    if [ -f "$PIDFILE" ]; then
        ...
        if command -v termux-notification-remove >/dev/null 2>&1; then
            termux-notification-remove fox_launcher 2>/dev/null
        fi
...
clear_cache_all() {
    ...
    if command -v termux-notification >/dev/null 2>&1; then
        termux-notification --id fox_cache --title "FOX Auto Clear Cache" \
...
cache_clear_running() {
    [ -f "$HOME/.fox_cache.pid" ] && kill -0 "$(cat "$HOME/.fox_cache.pid" 2>/dev/null)" 2>/dev/null
}
...
enable_cache_clear() {
    ...
    nohup bash -c "
        ...
        clear_cache_loop
    " >/dev/null 2>&1 &
    echo $! > "$HOME/.fox_cache.pid"
}

disable_cache_clear() {
    if [ -f "$HOME/.fox_cache.pid" ]; then
        kill "$(cat "$HOME/.fox_cache.pid" 2>/dev/null)" 2>/dev/null
        rm -f "$HOME/.fox_cache.pid"
    fi
    if command -v termux-notification-remove >/dev/null 2>&1; then
        termux-notification-remove fox_cache 2>/dev/null
    fi
}
...
ram_boost_once() {
    ...
    while read -r pkg; do
        ...
        # Lewati kalau masuk whitelist statis (sistem/termux/fox sendiri)
    ...
    if [ "$killed" -gt 0 ] && command -v termux-notification >/dev/null 2>&1; then
        termux-notification --id fox_ramboost --title "FOX RAM Booster" \
...
disable_ram_boost() {
    ...
    if command -v termux-notification-remove >/dev/null 2>&1; then
        termux-notification-remove fox_ramboost 2>/dev/null
    fi
}
